...
def login_screen():
    ...
    lbl_username = tk.Label(window, text="Tài khoản:") # Nhãn cho tài khoản
    lbl_username.pack()
...
def confirm_delete_referee(referee_name):
    ...
    manage_referees()
def manage_stadiums(): #qly sân thi đấu
    clear_screen()
...
def manage_users(): # Giao diện quản lý người dùng
    clear_screen()
...
def add_user():
    ...
    btn_back.pack(pady=5)
def save_new_user(username, password, role): # Hàm lưu người dùng mới vào file CSV
    if username and password:
        ...
        messagebox.showwarning("Lỗi", "Vui lòng nhập đầy đủ thông tin!")
def edit_user(): #  chỉnh sửa thông tin người dùng
    clear_screen()
    ...
    btn_back = tk.Button(window, text="Quay lại", command=manage_users)
    btn_back.pack(pady=5)
def save_edited_user(username, new_password): # Hàm lưu thông tin người dùng đã chỉnh sửa
    users = []   # Đọc tất cả người dùng từ file CSV
    try:
        with open('accounts.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
        ...
        messagebox.showerror("Lỗi", "Không tìm thấy file chứa người dùng.")
        return
    with open('accounts.csv', 'w', newline='') as file:     # Ghi lại thông tin người dùng vào file CSV
    ...
    manage_users()
def delete_user(): #hàm xoá ng dùng
    clear_screen()
...
def confirm_delete(username):
    ...
    manage_users()
def manage_teams(): #giao diện qly đội bóng
    clear_screen()
    ...
    btn_back = tk.Button(window, text="Quay lại", command=admin_dashboard)
    btn_back.pack(pady=5)
def view_teams(): # Hàm xem danh sách đội bóng
    try:
...
def save_edited_team(team_name, new_coach_name):
    ...
    try:
        ...
        messagebox.showerror("Lỗi", "Không tìm thấy file chứa đội bóng.")
        return
    with open('teams.csv', 'w', newline='') as file: # Ghi lại thông tin đội bóng vào file CSV
    ...
    manage_teams()
def delete_team(): #hàm xoá đội bóng
    clear_screen()
...
def view_schedule():
    try:
        with open('matches.csv', 'r',newline='') as file:
            reader = csv.DictReader(file, delimiter=';')
            headers = reader
            print("Tiêu đề cột:", headers)
            matches = [f"{row['Home Team']} vs {row['Away Team']} - {row['Referee']} - {row['Stadium']} {row['Date']} " for row in reader]
            match_list = "\n".join(matches)
        ...
        messagebox.showerror("Lỗi", f"Không tìm thấy cột: {e}")
def add_match():
    clear_screen()
    ...
    lbl_date_time = tk.Label(window, text="Ngày thi đấu ( dd/mm/yyyy )")
    lbl_date_time.pack()
    ...
    btn_back.pack(pady=5)
def save_new_match(team1, team2, referee, stadium, date):
    try:
        # Kiểm tra định dạng ngày
        date_obj = datetime.strptime(date, '%d/%m/%y')
        date = date_obj.strftime('%d/%m/%Y')  # Định dạng lại thành 'dd/mm/yyyy'
    except ValueError:
        messagebox.showwarning("Lỗi", "Ngày không đúng định dạng! Vui lòng nhập theo định dạng dd/mm/yyyy.")
        return
    if team1 and team2 and referee and stadium and date:
        with open('matches.csv', 'a', newline='') as file:
            writer = csv.writer(file, delimiter=';')
            writer.writerow([team1, team2, referee, stadium, date])  # Lưu thông tin trận đấu
